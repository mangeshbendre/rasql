...

{
    QMessageBox::about(this, tr("About RASQL"),
                       tr("RASQL (Relational Algebra Translator) V0.1.\nDeveloped by: Mangesh and Nilam."));
}
...
{
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save File"), "untitled.rasql", tr("RASQL files (*.rasql);;Plain Text files(*.txt)"));
    if (fileName.isEmpty())
        return false;
